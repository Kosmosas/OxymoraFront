function loadedFirst(){
	...
	// GLITCHING
	function initializeGlitch(image, height, width) {
		var container,
			stats;
		...
		function init(){
			...
			$(renderer.domElement).attr('id', 'loader').css('position','relative');
			$(renderer.domElement).attr('id', 'loader').css('bottom',height);
			$(renderer.domElement).attr('id', 'loader').css('opacity',0.5);
	...
	function loadPageCanvas() {
		...
		//initializeGlitch(canvasDataURL, screenShotCanvas.height,screenShotCanvas.width);
